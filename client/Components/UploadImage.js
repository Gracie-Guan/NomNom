...
export default function ImagePickerExample({ restaurantId, onPress }) {
    ...
    const addItem = async (data, menuId) => {
        ...
        console.log("UploadImage - json_data: ", json_data);

        console.log("json_data.restaurants[0].menu: ", json_data.restaurants[0]);

        // console.log("json_data.restaurants[0].menu.categories: ", json_data.restaurants[0].menu[0].categories);
    ...
    const fetchImageFromServer = async (imageUri) => {
        ...
        try {
            ...
            const { extracted_text, extracted_text_02, extracted_text_03 } = response.data;


            console.log("UploadImage - extracted_text, extracted_text_03: ", extracted_text, extracted_text_03);
            ...
            // if (JSON.parse(extracted_text)) {
            //     addItem(extracted_text, menu_info);
            // } else {
            //     addItem(extracted_text_02, menu_info);
            // }

            try {
                // Attempt to parse the first response
                console.log("UploadImage - extracted_text: ", extracted_text);
                extractedData = JSON.parse(extracted_text);
                console.log("Successfully parsed first response.");
                addItem(extracted_text, menu_info);

            } catch (error) {
                console.error("Failed to parse first response, trying the second one.", error);
            
                try {
                    // Attempt to parse the second response if the first fails
                    extractedData = JSON.parse(extracted_text_02);
                    console.log("Successfully parsed second response.");
                    addItem(extracted_text_02, menu_info);

                } catch (error) {
                    console.error("Failed to parse second response as well.", error);
                    // Handle the error accordingly, e.g., display an error message or log the issue
                    
                    try {
                        // Attempt to parse the second response if the first fails
                        extractedData = JSON.parse(extracted_text_03);
                        console.log("Successfully parsed third response.");
                        addItem(extracted_text_03, menu_info);
    
                    } catch (error) {
                        console.error("Failed to parse third response as well.", error);
                        // Handle the error accordingly, e.g., display an error message or log the issue
                        
                        Alert.alert('Error', 'Failed to parse any response');
